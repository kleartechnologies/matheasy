...
import 'package:matheasy/features/result/presentation/widgets/visual/engine/integral_power_rule_view.dart';
...
void main() {
  testWidgets('renders the integral walkthrough to the answer', (tester) async {
    final model = IntegralPowerRule.tryBuild(
      ...
      MaterialApp(
        ...
        home: Scaffold(
          body: SingleChildScrollView(
            child: IntegralPowerRuleView(model: model),
          ),
        ),
      ),
    );
    ...
    expect(find.text('Integrate with the power rule'), findsOneWidget);
    ...
    expect(find.text('The integral'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
}
