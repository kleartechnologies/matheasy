...
/// A Photomath-style PERCENTAGE tutorial (e.g. `15% of 80`): percent means "out
/// of 100", so rewrite as a fraction of 100 and multiply. Driven by the verified
/// [Percentage] model. The math line renders with the real renderer; the callout
/// is prose.
class PercentageView extends StatefulWidget {
  const PercentageView({super.key, required this.model, this.onAskStep});

  final Percentage model;
  final ValueChanged<int>? onAskStep;

  @override
  State<PercentageView> createState() => _PercentageViewState();
}

class _PercentageViewState extends State<PercentageView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
