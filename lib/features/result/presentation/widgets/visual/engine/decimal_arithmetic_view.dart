...
/// A Photomath-style DECIMAL ARITHMETIC tutorial (e.g. `3.2 + 1.45`, `0.5 × 4`):
/// line up the decimal points for +/−, or "multiply then place the point" for ×.
/// Driven by the verified [DecimalArithmetic] model. The math line renders with
/// the real renderer; the callout is prose.
class DecimalArithmeticView extends StatefulWidget {
  const DecimalArithmeticView({super.key, required this.model, this.onAskStep});

  final DecimalArithmetic model;
  final ValueChanged<int>? onAskStep;

  @override
  State<DecimalArithmeticView> createState() => _DecimalArithmeticViewState();
}

class _DecimalArithmeticViewState extends State<DecimalArithmeticView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
