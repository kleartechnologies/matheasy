...
/// A Photomath-style LOGARITHM tutorial (e.g. `log_2(8)`): a log asks "what power
/// of the base gives the number?". Driven by the verified [Logarithm] model. The
/// math line renders with the real renderer; the callout is prose.
class LogarithmView extends StatefulWidget {
  const LogarithmView({super.key, required this.model, this.onAskStep});

  final Logarithm model;
  final ValueChanged<int>? onAskStep;

  @override
  State<LogarithmView> createState() => _LogarithmViewState();
}

class _LogarithmViewState extends State<LogarithmView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
