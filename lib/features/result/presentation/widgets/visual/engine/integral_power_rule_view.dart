...
/// A Photomath-style INTEGRAL (power rule) tutorial (e.g. `∫(3x^2+4x-5)dx`):
/// add one to each power, divide by the new power, add C. Driven by the verified
/// [IntegralPowerRule] model; the callout is math.
class IntegralPowerRuleView extends StatefulWidget {
  const IntegralPowerRuleView({super.key, required this.model, this.onAskStep});

  final IntegralPowerRule model;
  final ValueChanged<int>? onAskStep;

  @override
  State<IntegralPowerRuleView> createState() => _IntegralPowerRuleViewState();
}

class _IntegralPowerRuleViewState extends State<IntegralPowerRuleView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
