...
/// A Photomath-style DERIVATIVE (power rule) tutorial (e.g.
/// `d/dx(x^3 + 2x^2 - 5x + 1)`): bring each power down as a multiplier and drop
/// it by one. Driven by the verified [DerivativePowerRule] model; callout is math.
class DerivativePowerRuleView extends StatefulWidget {
  const DerivativePowerRuleView({super.key, required this.model, this.onAskStep});

  final DerivativePowerRule model;
  final ValueChanged<int>? onAskStep;

  @override
  State<DerivativePowerRuleView> createState() =>
      _DerivativePowerRuleViewState();
}

class _DerivativePowerRuleViewState extends State<DerivativePowerRuleView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
  ...
  void _schedule() {
    ...
    _timer = Timer(const Duration(milliseconds: 1800), () => _go(_index + 1));
  }
