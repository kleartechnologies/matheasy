...
/// A Photomath-style QUADRATIC FORMULA tutorial (e.g. `x^2 - 5x + 6 = 0`):
/// identify a, b, c → the formula → substitute → discriminant → two roots.
/// Driven by the verified [QuadraticFormula] model; the callout is math.
class QuadraticFormulaView extends StatefulWidget {
  const QuadraticFormulaView({super.key, required this.model, this.onAskStep});

  final QuadraticFormula model;
  final ValueChanged<int>? onAskStep;

  @override
  State<QuadraticFormulaView> createState() => _QuadraticFormulaViewState();
}

class _QuadraticFormulaViewState extends State<QuadraticFormulaView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
