...
/// A Photomath-style 2×2 DETERMINANT tutorial: main diagonal minus anti-diagonal
/// (ad − bc). Driven by the verified [MatrixDeterminant] model; callout is math.
class MatrixDeterminantView extends StatefulWidget {
  const MatrixDeterminantView({super.key, required this.model, this.onAskStep});

  final MatrixDeterminant model;
  final ValueChanged<int>? onAskStep;

  @override
  State<MatrixDeterminantView> createState() => _MatrixDeterminantViewState();
}

class _MatrixDeterminantViewState extends State<MatrixDeterminantView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
  ...
  void _schedule() {
    ...
    _timer = Timer(const Duration(milliseconds: 1700), () => _go(_index + 1));
  }
