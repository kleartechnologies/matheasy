...
/// A Photomath-style FRACTION ARITHMETIC tutorial (e.g. `1/2 + 1/3`): steps
/// through the standard method — common denominator, rewrite, combine, simplify —
/// rendering each expression with the real fraction renderer and a bold callout
/// for the key move. Driven by the verified [FractionArithmetic] model.
class FractionArithmeticView extends StatefulWidget {
  const FractionArithmeticView({super.key, required this.model, this.onAskStep});

  final FractionArithmetic model;
  final ValueChanged<int>? onAskStep;

  @override
  State<FractionArithmeticView> createState() => _FractionArithmeticViewState();
}

class _FractionArithmeticViewState extends State<FractionArithmeticView> {
  int _index = 0;
  Timer? _timer;
  bool _playing = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // The current expression, rendered big. Cross-fades between steps.
        SizedBox(
          height: 96,
          child: Center(
