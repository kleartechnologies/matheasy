...
{"kind":"sasArea", "unknown":"Area", "sides":[{"label":"a","value":11},{"label":"b","value":13}], "includedAngle":{"label":"C","value":39}}
   (the angle must sit BETWEEN the two given sides, and "unknown" MUST be the literal string "Area" — if the problem asks for a SIDE of that triangle instead, use kind 7 (cosineRuleSide).)
6. Right triangle, TWO sides given, an acute ANGLE unknown (inverse trig — SOH CAH TOA read backwards):
{"kind":"rightTriangleInverseTrig", "unknown":"x", "sides":[{"label":"a","role":"opposite","value":10},{"label":"c","role":"hypotenuse","value":24}]}
   (EXACTLY 2 sides, BOTH with a value; the unknown is the ASKED ANGLE (its "unknown" is the angle's label, e.g. the vertex the question names — never a side). "role" is opposite|adjacent|hypotenuse RELATIVE TO the ASKED angle: opposite = the side across from it, adjacent = the LEG touching it (never the hypotenuse), hypotenuse = the side across from the right angle. Use THIS (not sineRuleAngle) whenever the triangle has a RIGHT angle and two side lengths are given. If the figure is a COMPOUND figure with more than one triangle, OMIT geometry.)
7. Any triangle, TWO sides + the INCLUDED angle between them given, the THIRD side unknown (cosine rule):
{"kind":"cosineRuleSide", "unknown":"x", "sides":[{"label":"a","value":8},{"label":"b","value":5}], "includedAngle":{"label":"C","value":60}}
   (EXACTLY 2 sides + the angle BETWEEN them; the unknown is the side OPPOSITE that angle. Use this for a non-right triangle where two sides and the angle between them are given and a side is asked. NOT for a right triangle (use Pythagoras/rightTriangleTrig).)
8. Any triangle, ALL THREE sides given, an ANGLE unknown (cosine rule):
{"kind":"cosineRuleAngle", "unknown":"A", "sides":[{"label":"a","value":7},{"label":"b","value":8},{"label":"c","value":9}], "oppositeSide":"a"}
   (EXACTLY 3 sides, all with a value; the unknown is the ASKED angle, and "oppositeSide" is the LABEL of the side directly ACROSS FROM that angle — the other two sides sit around it. Use this whenever all three sides are known and an angle is asked.)
CRITICAL for geometry: every "value" is a plain NUMBER — angle values in degrees WITHOUT the ° symbol, lengths without units. Read the GIVEN numbers off the figure/text and name the single unknown — DO NOT compute or include the unknown's value; the app computes it deterministically and discards your geometry object if the numbers are inconsistent. OMIT "geometry" entirely for anything else (proofs, perimeter, other area shapes, coordinate geometry, circle theorems, a COMPOUND figure with more than one triangle, or any problem you're unsure about) — a wrong geometry object is worse than none. "problem" must ALWAYS still contain the full transcription regardless (including the figure's given numbers when the text alone doesn't carry them).
